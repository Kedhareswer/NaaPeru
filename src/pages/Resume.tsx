...
const Resume = () => {
  ...
  // A4 size in CSS pixels for screen rendering (8.27in x 11.69in at 96dpi)
  const A4_WIDTH = 794;
  ...
  useEffect(() => {
    const handleResize = () => {
      const paddingX = 16 * 2; // px left+right small padding
      const paddingY = 24 * 2; // px top+bottom around the canvas area
      const navReserve = 96; // approx nav height space
      const availableWidth = Math.max(320, window.innerWidth - paddingX);
      const availableHeight = Math.max(320, window.innerHeight - (paddingY + navReserve));
  ...
  return (
    <div className="relative min-h-screen bg-[#1a1a1a]">
      ...
      <main className="relative z-10 flex items-center justify-center px-2 sm:px-4 py-24 sm:py-32">
        {/* Scaled Canvas Wrapper (reserves layout space equal to scaled size) */}
        <div style={{ width: scaledWidth, height: scaledHeight }}>
          {/* A4 Paper Canvas */}
          <div
            className="a4-paper relative bg-[#f5f5f0] text-[#1a1a1a] shadow-2xl p-12"
            style={{
              width: `${A4_WIDTH}px`,
              height: `${A4_HEIGHT}px`,
          ...
          >
            ...
            <div className="no-print absolute top-8 right-8">
              <a
                ...
                download
                className="flex items-center gap-2 border-2 border-[#ff0000] bg-transparent px-4 py-2 font-body text-xs uppercase tracking-[0.2em] text-[#ff0000] hover:bg-[#ff0000] hover:text-white transition-all"
              >
            ...
            <div className="mb-8 border-b-2 border-[#1a1a1a] pb-6">
              ...
              <h1 className="mt-4 font-heading text-5xl font-bold uppercase tracking-tight text-[#ff0000] leading-none">
                MARLAKUNTA<br />KEDHARESWER<br />NAIDU
              </h1>
              <div className="mt-4 flex flex-wrap gap-4 font-body text-xs text-[#666]">
