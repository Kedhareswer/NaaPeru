/**
 * Parameter-driven frame control, adapted from the oil-motion runtime.
 * Input (scroll/pointer) maps to a target frame; smoothDamp with a max speed
 * chases it; the render callback fires only when the integer frame changes.
 * No autoplay anywhere — the parameter owns the timeline.
 */
...
/**
 * Renders animator frames by seeking an all-keyframe video. Seeks are
 * serialized: while the decoder is mid-seek the latest frame is parked and
 * flushed on `seeked`, so fast scrubbing never queues a backlog.
 */
export function createVideoSeekRenderer(video: HTMLVideoElement, fps: number) {
