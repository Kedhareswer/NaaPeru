             ...
             <div className="w-full h-full bg-white dark:bg-gray-900 overflow-hidden">
               ...
               <div className="h-full overflow-y-auto">
                 ...
                 <div className="relative w-full bg-gray-100 dark:bg-gray-800">
                   ...
                   {activeTab.src && (
                     <img src={activeTab.src} alt={activeTab.title} className="w-full h-[280px] object-cover" />
                   )}
                   ...
                   <div className="absolute inset-x-0 bottom-0">
                     <div className="max-w-screen-2xl mx-auto px-6 py-4">
                       <div className="flex items-end justify-between gap-4">
                         <div>
                           <h1 className="text-2xl font-semibold text-white">{activeTab.title}</h1>
                           <div className="mt-3 flex flex-wrap gap-2">
                             ...
                             {activeTab.technologies?.length ? (
                               <span className="inline-flex items-center rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-gray-900">
                                 {activeTab.technologies.length} Tech
                               </span>
                             ) : null}
                           </div>
                         </div>
                         <div className="hidden md:flex gap-3">
                           {activeTab.demo && (
                             <a href={activeTab.demo} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-md bg-white/90 px-4 py-2 text-xs font-medium text-gray-900 hover:bg-white">
                               Live Demo
                             </a>
                           )}
                           {activeTab.github && (
                             <a href={activeTab.github} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-md bg-white/90 px-4 py-2 text-xs font-medium text-gray-900 hover:bg-white">
                               Source Code
                             </a>
                           )}
                         </div>
                  ...
                  <div className="max-w-screen-2xl mx-auto px-6 py-8">
                   <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                     ...
                     <div className="xl:col-span-8 space-y-8">
                       ...
                       {/* Objectives & Outcomes */}
                       {(activeTab.objectives?.length || activeTab.outcomes?.length) ? (
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                           {activeTab.objectives && activeTab.objectives.length > 0 && (
                             <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                               <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-800/20 px-6 py-4 border-b border-emerald-200 dark:border-emerald-700">
                                 <h3 className="text-lg font-semibold text-emerald-800 dark:text-emerald-200">Objectives</h3>
                               </div>
                               <ul className="p-6 space-y-3">
                                 {activeTab.objectives.map((item, idx) => (
                           ...
                           {activeTab.outcomes && activeTab.outcomes.length > 0 && (
                             <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                               <div className="bg-gradient-to-r from-sky-50 to-sky-100 dark:from-sky-900/20 dark:to-sky-800/20 px-6 py-4 border-b border-sky-200 dark:border-sky-700">
                                 <h3 className="text-lg font-semibold text-sky-800 dark:text-sky-200">Outcomes</h3>
                               </div>
                               <ul className="p-6 space-y-3">
                                 {activeTab.outcomes.map((item, idx) => (
                                   <li key={idx} className="flex items-start gap-3 text-gray-700 dark:text-gray-300">
                                     <span className="mt-1 inline-block w-2 h-2 rounded-full bg-sky-500"></span>
                                     <span>{item}</span>
                                   </li>
                                 ))}
                               </ul>
                     ...
                     <div className="xl:col-span-4 space-y-6">
                       ...
                       {activeTab.src && (
                         <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                           <div className="bg-gradient-to-r from-pink-50 to-pink-100 dark:from-pink-900/20 dark:to-pink-800/20 px-4 py-3 border-b border-pink-200 dark:border-pink-700">
                             <div className="flex items-center gap-2">
                               <span className="text-sm font-semibold text-pink-800 dark:text-pink-200">Project Media</span>
                             </div>
                           </div>
                           <div className="p-0">
                             {/* eslint-disable-next-line @next/next/no-img-element */}
                             <img src={activeTab.src} alt={activeTab.title} className="w-full h-64 object-cover" />
                           </div>
                         </div>
                       )}
                       {/* Quick Actions */}
                       ...
                       <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                         ...
                         <div className="p-4">
                           <div className="space-y-2">
                              {activeTab.technologies && activeTab.technologies.slice(0, 12).map((tech, index) => (
                               <div key={index} className="flex items-center gap-2">
                                 <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
                                 <span className="text-sm text-gray-700 dark:text-gray-300">{tech}</span>
                               </div>
                             ))}
                              {activeTab.technologies && activeTab.technologies.length > 12 && (
                               <div className="text-xs text-gray-500 dark:text-gray-400 pt-2 border-t border-gray-200 dark:border-gray-600">
                                  +{activeTab.technologies.length - 12} more technologies
                               </div>
                             )}
                           </div>
                         </div>
